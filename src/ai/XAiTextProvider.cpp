...
void XAiTextProvider::LoadConfig(const nlohmann::json& json)
{
    ...
    if (json.contains("vision_model")) m_VisionModel = json["vision_model"];
    if (json.contains("sentence_model")) m_SentenceModel = json["sentence_model"];
                                                  ...
                                                  Language::ILanguage* language)
{
    ...
    nlohmann::json payload = {
        ...
        {"messages", nlohmann::json::array({
            {
                {"role", "system"},
                {"content", language->GetOCRSystemPrompt()}
            },
            {
                {"role", "user"},
...
#ifndef NDEBUG
    // Create a copy for logging to avoid dumping the massive base64 string
    nlohmann::json debugPayload = payload;
    try {
        // Navigate to the image URL safely to truncate it
        auto& messages = debugPayload["messages"];
        if (messages.is_array() && messages.size() > 1) {
            auto& userContent = messages[1]["content"];
            if (userContent.is_array() && !userContent.empty()) {
                auto& imagePart = userContent[0];
                if (imagePart.contains("image_url") && imagePart["image_url"].contains("url")) {
                    std::string url = imagePart["image_url"]["url"];
                    if (url.length() > 64) {
                        imagePart["image_url"]["url"] = std::format("{}...[truncated, total_len={}]", url.substr(0, 32), url.length());
                    }
                }
            }
        }
        AF_DEBUG("Sending OCR Request: {}", debugPayload.dump(2));
    } catch (const std::exception& e) {
        AF_WARN("Failed to format debug payload for logging: {}", e.what());
    }
#endif
    ...
    if (!response.is_null() && response.contains("choices") && !response["choices"].empty())
    {
        ...
        {
            AF_INFO("xAI Token Usage (OCR): Prompt={}, Completion={}, Total={}",
                response["usage"].value("prompt_tokens", 0),
                response["usage"].value("completion_tokens", 0),
                response["usage"].value("total_tokens", 0));

            if (response["usage"].contains("prompt_image_tokens"))
            {
                AF_INFO("xAI Image Token Usage: {}", response["usage"].value("prompt_image_tokens", 0));
            }
        }
        ...
        if (lastBackticks != std::string::npos && lastBackticks > start) {
            ...
            // Also trim trailing newline before ```
                                               ...
                                               Language::ILanguage* language)
{
    ...
    nlohmann::json payload = {
        ...
        {"messages", nlohmann::json::array({
            {
                {"role", "system"},
                {"content", language->GetAnalysisSystemPrompt()}
            },
            {
                {"role", "user"},
                {"content", prompt}
