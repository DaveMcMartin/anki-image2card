...
void ElevenLabsAudioProvider::LoadConfig(const nlohmann::json& json)
{
    ...
    if (json.contains("voice_id")) m_VoiceId = json["voice_id"];
...
void ElevenLabsAudioProvider::LoadRemoteVoices()
{
    ...
    try
    {
        ...
        if (res && res->status == 200)
        {
            ...
            std::sort(m_AvailableVoices.begin(), m_AvailableVoices.end(),
                [](const ElevenLabsVoice& a, const ElevenLabsVoice& b) {
...
std::vector<unsigned char> ElevenLabsAudioProvider::GenerateAudio(const std::string& text, const std::string& voiceId, const std::string& languageCode)
{
