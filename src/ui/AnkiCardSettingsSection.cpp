...
AnkiCardSettingsSection::AnkiCardSettingsSection(SDL_Renderer* renderer, API::AnkiConnectClient* ankiConnectClient, Config::ConfigManager* configManager)
    : m_Renderer(renderer)
...
void AnkiCardSettingsSection::SetFieldByTool(int toolIndex, const std::vector<unsigned char>& data, const std::string& filename)
{
    for (auto& field : m_Fields)
    {
        if (field->IsToolEnabled() && field->GetSelectedToolIndex() == toolIndex)
...
{
    ImGui::BeginChild("FieldsRegion", ImVec2(0, -40), false, 0);
    ...
    for (auto& field : m_Fields)
    {
        ...
        if (wasEnabled != field->IsToolEnabled() || oldTool != field->GetSelectedToolIndex())
        {
            if (m_ConfigManager && !m_NoteTypes.empty())
            {
                ...
                m_ConfigManager->GetConfig().FieldMappings[currentNoteType][field->GetName()].second = field->GetSelectedToolIndex();
                m_ConfigManager->Save();
            }
        }
...
void AnkiCardSettingsSection::CheckDuplicatesAndAdd()
{
    ...
    for (const auto& field : m_Fields)
    {
        ...
        bool isSentence = (field->IsToolEnabled() && field->GetSelectedToolIndex() == 0) || field->GetName() == "Sentence" || field->GetName() == "Expression";
        bool isVocab = (field->IsToolEnabled() && field->GetSelectedToolIndex() == 3) || field->GetName() == "Target Word" || field->GetName() == "Vocab Word";

        if ((isSentence || isVocab) && !field->GetValue().empty())
        {
            if (hasCriteria) query += " OR ";
    ...
    if (!hasCriteria)
    {
        ...
        for (const auto& field : m_Fields) if (!field->GetValue().empty()) anyFieldFilled = true;
    ...
    if (!notes.empty())
    {
        m_DuplicateMessage = "Found " + std::to_string(notes.size()) + " duplicate note(s) in deck '" + deckName + "'.\nAdd anyway?";
...
void AnkiCardSettingsSection::PerformAdd()
{
    ...
    int64_t noteId = m_AnkiConnectClient->AddNote(deckName, modelName, fieldsMap, {"image2card"});
    if (noteId > 0)
    {
        m_LastCardId = noteId;
        AF_INFO("Note added successfully. Card ID: {}", noteId);
        if (m_OnStatusMessage) m_OnStatusMessage("Note added successfully.");
    ...
    else
    {
        ...
        if (m_OnStatusMessage) m_OnStatusMessage("Failed to add note.");
    }
}
