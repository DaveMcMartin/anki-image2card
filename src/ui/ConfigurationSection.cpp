...
void ConfigurationSection::RenderTextAITab()
{
    ...
    if (*m_ActiveTextAIProvider)
    {
        ...
        if ((*m_ActiveTextAIProvider)->RenderConfigurationUI())
        {
            ...
            if (providerId == "xai")
            {
                if (j.contains("api_key")) config.TextApiKey = j["api_key"];
                if (j.contains("vision_model")) config.TextVisionModel = j["vision_model"];
                if (j.contains("sentence_model")) config.TextSentenceModel = j["sentence_model"];
                if (j.contains("available_models")) config.TextAvailableModels = j["available_models"].get<std::vector<std::string>>();
            }
            else if (providerId == "google")
            {
                if (j.contains("api_key")) config.GoogleApiKey = j["api_key"];
                if (j.contains("model")) config.GoogleModel = j["model"];
                if (j.contains("available_models")) config.GoogleAvailableModels = j["available_models"].get<std::vector<std::string>>();
            }
    ...
    if (m_AudioAIProvider)
    {
        ...
        if (m_AudioAIProvider->RenderConfigurationUI())
        {
            ...
            if (m_ConfigManager)
            {
                ...
                if (j.contains("api_key")) config.AudioApiKey = j["api_key"];
                if (j.contains("voice_id")) config.AudioVoiceId = j["voice_id"];
