...
void CardField::Render(SDL_Renderer* renderer)
{
    ...
    if (m_IsToolEnabled)
    {
        ...
        if (ImGui::BeginCombo("##ToolSelect", Image2Card::Core::FieldToolNames[m_SelectedToolIndex].data()))
        {
            for (int i = 0; i < static_cast<int>(Image2Card::Core::FieldToolNames.size()); i++)
            {
                const bool isSelected = (m_SelectedToolIndex == i);
                if (ImGui::Selectable(Image2Card::Core::FieldToolNames[i].data(), isSelected))
                {
...
void CardField::RenderTypeSelector()
{
    ...
    if (isText) ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.26f, 0.59f, 0.98f, 1.0f));
    if (ImGui::Button(ICON_FA_FONT)) SetType(CardFieldType::Text);
    if (ImGui::IsItemHovered()) ImGui::SetTooltip("Text");
    if (isText) ImGui::PopStyleColor();

    ImGui::SameLine();

    if (isAudio) ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.26f, 0.59f, 0.98f, 1.0f));
    if (ImGui::Button(ICON_FA_MUSIC)) SetType(CardFieldType::Audio);
    if (ImGui::IsItemHovered()) ImGui::SetTooltip("Audio");
    if (isAudio) ImGui::PopStyleColor();

    ImGui::SameLine();

    if (isImage) ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.26f, 0.59f, 0.98f, 1.0f));
    if (ImGui::Button(ICON_FA_IMAGE)) SetType(CardFieldType::Image);
    if (ImGui::IsItemHovered()) ImGui::SetTooltip("Image");
    ...
    auto UpdateTexture = [this, renderer]() {
        ...
        if (!m_BinaryData.empty())
        {
            ...
            unsigned char* data = stbi_load_from_memory(
                m_BinaryData.data(),
                static_cast<int>(m_BinaryData.size()),
                &width, &height, &channels, 4
            );

            if (data)
            {
                auto surface = SDL::MakeSurfaceFrom(
                    width, height,
