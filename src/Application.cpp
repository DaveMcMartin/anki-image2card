...
bool Application::Initialize()
{
    ...
    auto window_flags = (SDL_WindowFlags)(SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE | SDL_WINDOW_HIDDEN | SDL_WINDOW_HIGH_PIXEL_DENSITY);
...

    {
        ...
        if (iconData)
        {
            auto iconSurface = SDL::MakeSurfaceFrom(
                iconWidth,
                iconHeight,
    ...
    if (ankiUrl.empty()) ankiUrl = "http://localhost:8765";
    m_AnkiConnectClient = std::make_unique<API::AnkiConnectClient>(ankiUrl);

    m_ImageSection = std::make_unique<UI::ImageSection>(
        m_Renderer,
        &m_Languages,
        &m_ActiveLanguage,
        m_ConfigManager.get()
    ...
    m_AnkiCardSettingsSection = std::make_unique<UI::AnkiCardSettingsSection>(m_Renderer, m_AnkiConnectClient.get(), m_ConfigManager.get());
    ...
    m_AnkiCardSettingsSection->SetOnStatusMessageCallback([this](const std::string& msg) {
        if (m_StatusSection) m_StatusSection->SetStatus(msg);
    });
    ...
    m_ConfigurationSection->SetOnConnectCallback([this]() {
        ...
        if (m_StatusSection) m_StatusSection->SetStatus("AnkiConnect: Connected");
    });

    m_ImageSection->SetOnScanCallback([this]() {
        OnScan();
    });
    ...
    std::thread([this]() {
        ...
        else
        {
            ...
            if (m_StatusSection) m_StatusSection->SetStatus("AnkiConnect: Not connected (click Connect to retry)");
        }
    }).detach();
...
void Application::RenderUI()
{
    ...
    if (first_time)
    {
        ...
        ImGui::DockBuilderAddNode(dockspace_id, ImGuiDockNodeFlags_DockSpace | ImGuiDockNodeFlags_NoWindowMenuButton | ImGuiDockNodeFlags_NoCloseButton);
        ...
        ImGuiDockNode* node;
        if ((node = ImGui::DockBuilderGetNode(dock_left_id))) node->LocalFlags |= ImGuiDockNodeFlags_NoTabBar | ImGuiDockNodeFlags_NoWindowMenuButton | ImGuiDockNodeFlags_NoCloseButton;
        if ((node = ImGui::DockBuilderGetNode(dock_right_id))) node->LocalFlags |= ImGuiDockNodeFlags_NoTabBar | ImGuiDockNodeFlags_NoWindowMenuButton | ImGuiDockNodeFlags_NoCloseButton;
        if ((node = ImGui::DockBuilderGetNode(dock_bottom_id))) node->LocalFlags |= ImGuiDockNodeFlags_NoTabBar | ImGuiDockNodeFlags_NoWindowMenuButton | ImGuiDockNodeFlags_NoCloseButton;
...
void Application::OnScan()
{
    if (m_IsScanning.load())
    {
        ...
        if (m_StatusSection) m_StatusSection->SetStatus("Scan already in progress.");
        return;
    }

    AF_INFO("Starting Scan...");
    if (m_StatusSection) m_StatusSection->SetStatus("Scanning image...");
    ...
    {
        if (m_StatusSection) m_StatusSection->SetStatus("Error: Anki is not connected.");
    ...
    if (imageBytes.empty())
    {
        if (m_StatusSection) m_StatusSection->SetStatus("Error: No image selected.");
    ...
    if (m_StatusSection) m_StatusSection->SetProgress(0.0f);
    ...
    task.onComplete = [this, scanImage = std::move(scanImage)]() {
        ...
        if (m_StatusSection) m_StatusSection->SetProgress(-1.0f);
        ...
        if (!error.empty())
        {
            if (m_StatusSection) m_StatusSection->SetStatus("Error: " + error);
        ...
        if (ocrResult.empty())
        {
            if (m_StatusSection) m_StatusSection->SetStatus("Error: OCR returned no text.");
        ...
        if (m_StatusSection) m_StatusSection->SetStatus("Scan complete.");
...
void Application::RenderScanModal()
{
    ...
    if (ImGui::BeginPopupModal("Scan Result", &m_ShowScanModal, ImGuiWindowFlags_AlwaysAutoResize))
    {
        auto InputText = [](const char* label, std::string* str) {
            ...
            return ImGui::InputText(label, (char*)str->data(), str->capacity() + 1, ImGuiInputTextFlags_CallbackResize, callback, (void*)str);
        };
...
void Application::ProcessScan()
{
    ...
    if (m_IsProcessing.load())
    {
        ...
        if (m_StatusSection) m_StatusSection->SetStatus("Processing already in progress.");
        return;
    }

    AF_INFO("Processing Scan. Sentence: '{}', Target Word: '{}'", m_ScanSentence, m_ScanTargetWord);
    if (m_StatusSection) m_StatusSection->SetStatus("Processing scan...");
    ...
    if (m_StatusSection) m_StatusSection->SetProgress(0.1f);
    ...
    task.future = std::async(std::launch::async, [this, sentence, targetWord, voice, fullImage, languageCode]() {
        try
        {
            ...
            if (m_StatusSection) m_StatusSection->SetProgress(0.4f);
            ...
            auto updateFields = [this, analyzedSentence, translation, analyzedTargetWord, targetWordFurigana, furigana, definition, pitch, fullImage]() {
                if (m_AnkiCardSettingsSection)
                {
            ...
            {
                if (m_StatusSection) m_StatusSection->SetProgress(0.6f);
                AF_INFO("Generating Vocab Audio for: {}", analyzedTargetWord);
                if (m_StatusSection) m_StatusSection->SetStatus("Generating Vocab Audio...");

                std::vector<unsigned char> vocabAudio = m_AudioAIProvider->GenerateAudio(analyzedTargetWord, voice, languageCode);
            ...
            {
                if (m_StatusSection) m_StatusSection->SetProgress(0.8f);
                AF_INFO("Generating Sentence Audio for: {}", analyzedSentence);
                if (m_StatusSection) m_StatusSection->SetStatus("Generating Sentence Audio...");

                std::vector<unsigned char> sentenceAudio = m_AudioAIProvider->GenerateAudio(analyzedSentence, voice, languageCode);
    ...
    task.onComplete = [this]() {
        ...
        if (m_StatusSection) m_StatusSection->SetStatus("Processing complete.");
